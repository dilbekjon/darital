...
export default function AdminUsersPage() {
  ...
  const handleSaveCredentials = async () => {
    ...
    setError(null);
    try {
      const endpoint = credentialsUser.id === user?.id
        ? '/admin/users/me/credentials'
        : `/admin/users/${credentialsUser.id}/credentials`;
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ phone: '', password: '', fullName: '', role: AdminRole.ADMIN });
      setError(null);
    } catch (err) {
