...
export function AdminSidebar() {
  ...
  const allMenuItems: SidebarMenuItem[] = [
    ...
    },
    {
      ...
      href: '/admin/utility-bills',
      permissionCodes: ['utility.bills.read'],
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const sidebarContent = (
    <div className="p-4">
      ...
      <div className={`mt-6 pt-4 border-t ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
        <Link href="/docs" onClick={() => setIsMobileOpen(false)} className={`flex items-center gap-3 py-2 px-4 rounded-lg text-sm ${darkMode ? 'text-gray-400 hover:text-blue-400 hover:bg-blue-600/10' : 'text-gray-500 hover:text-blue-600 hover:bg-gray-100'} transition-colors`}>
